...
class SlackIntegration:
    ...
    def interactive_setup(self) -> bool:
        ...
        print("  python .claude/scripts/slack-integration.py test")
...
status=$?
if [ $status -eq 0 ]; then
    [ "{errors_only}" != "True" ] && python .claude/scripts/slack-notify.py "✅ Build succeeded"
else
    python .claude/scripts/slack-notify.py "❌ Build failed" --mention
fi
'''
...
status=$?
if [ $status -eq 0 ]; then
    [ "{errors_only}" != "True" ] && python .claude/scripts/slack-notify.py "✅ Tests passed"
else
    python .claude/scripts/slack-notify.py "❌ Tests failed" --mention
fi
'''
...
command = '''
python .claude/scripts/slack-notify.py "🏃 Daily Standup Reminder" --template standup
'''
...
command = '''
python .claude/scripts/slack-notify.py "🔍 New PR ready for review" --template pr_created
'''
...
if echo "$CLAUDE_OUTPUT" | grep -qiE "(error|failed|exception|fatal)"; then
    python .claude/scripts/slack-notify.py "⚠️ Error detected in command output" --error "$CLAUDE_OUTPUT"
fi
'''
...
def load_config():
    ...
    if not config_path.exists():
        print("Error: Slack not configured. Run: python .claude/scripts/slack-integration.py")
        sys.exit(1)
...
'''
        ...
        script_path = Path(".claude/scripts/slack-notify.py")
