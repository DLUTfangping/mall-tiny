...
class EmailIntegration:
    ...
    def interactive_setup(self) -> bool:
        ...
        print("  python .claude/scripts/email-integration.py test")
...
time = "18:00"
command = '''
python .claude/scripts/email-notify.py --template daily_summary
'''
...
days = ["friday"]
command = '''
python .claude/scripts/email-notify.py --template weekly_report
'''
...
if [ $status -ne 0 ] || [ "{critical_only}" != "True" ]; then
    python .claude/scripts/email-notify.py --template build_status --status $status
fi
'''
...
if echo "$CLAUDE_OUTPUT" | grep -qiE "(error|failed|exception|fatal|critical)"; then
    python .claude/scripts/email-notify.py --template error_alert --error "$CLAUDE_OUTPUT" --immediate
fi
'''
...
command = '''
python .claude/scripts/email-notify.py --template sprint_summary
'''
...
def load_config():
    ...
    if not config_path.exists():
        print("Error: Email not configured. Run: python .claude/scripts/email-integration.py")
        sys.exit(1)
...
'''
        ...
        script_path = Path(".claude/scripts/email-notify.py")
