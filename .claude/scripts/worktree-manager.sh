...
check_repo() {
    ...
    if [[ ! -d "$PROJECTS_DIR/$repo" ]]; then
        echo "Error: Repository '$repo' not found in $PROJECTS_DIR"
        return 1
    fi
...
create_feature() {
    ...
    for repo in "${repos[@]}"; do
        ...
        if [[ -d "$worktree_path" ]]; then
            echo "Warning: Worktree already exists: $worktree_path"
            continue
        fi
        ...
        if git worktree add "$worktree_path" -b "$branch_name" "origin/$DEFAULT_BASE_BRANCH" 2>/dev/null; then
            ...
            if git show-ref --verify --quiet "refs/heads/$branch_name"; then
                ...
                echo "Created worktree with existing branch: $worktree_path"
            else
    ...
    if [[ ${#created_worktrees[@]} -gt 0 ]]; then
        echo ""
        echo "Successfully created ${#created_worktrees[@]} worktree(s) for feature: $feature_name"
...
cleanup_merged() {
            ...
            for worktree in "$repo_dir"/*; do
                if [[ -d "$worktree" ]]; then
                    ...
                    if echo "$merged_branches" | grep -q "^$branch_name$"; then
                        echo "Removing merged worktree: $repo/$feature"
    ...
    if [[ $removed_count -gt 0 ]]; then
        echo ""
        echo "Removed $removed_count merged worktree(s)"
    else
        echo ""
        echo "No merged worktrees found to cleanup"
    fi
}
...
feature_status() {
    ...
    echo "Status for feature: $feature_name"
    echo

    local tracking_file="$WORKTREES_DIR/.feature-$feature_name"

    if [[ ! -f "$tracking_file" ]]; then
        echo "No tracking information found for feature: $feature_name"
        return 1
    fi
...
remove_worktree() {
    ...
    echo "Removing worktree: $repo/$feature_name"
...
sync_status() {
    ...
    echo "Syncing worktree status with GitHub..."
    echo
    ...
    echo "GitHub sync functionality coming soon!"
...
case "${1:-}" in
    create-feature)
        shift
        if [[ $# -lt 2 ]]; then
            echo "Error: Feature name and at least one repository required"
            usage
            exit 1
        fi
        create_feature "$@"
    ...
    remove-worktree)
        shift
        if [[ $# -ne 2 ]]; then
            echo "Error: Repository and feature name required"
            usage
            exit 1
        fi
        remove_worktree "$1" "$2"
