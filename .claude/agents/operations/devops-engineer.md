  ...
  deploy:
    ...
    steps:
      ...
      - name: Run Smoke Tests
        run: |
          ./.claude/scripts/smoke-tests.sh
          
      - name: Notify Success
        if: success()
        run: |
          ./.claude/scripts/notify-deployment.sh success
```
...
3. Enable circuit breaker: `./.claude/scripts/enable-circuit-breaker.sh`
...
- Failover: `./.claude/scripts/failover-to-secondary.sh`
...
**When to use:** When configuring CI/CD pipelines for Bun-based projects, setting up Bun in containerized environments, optimizing build times with bun install, or deploying Bun applications. Triggers: Bun, bun.sh, bun install, bun build, bun test, Bun runtime, Bun CI/CD.

**How to use:** This skill provides Bun deployment and CI/CD patterns. When working with Bun in pipelines:
1. Use `bun install --frozen-lockfile` for reproducible builds
2. Leverage Bun's 30x faster package installation for CI speedup
3. Configure GitHub Actions with `oven-sh/setup-bun@v2`
4. Use `bun build --compile` for containerized deployments

**Skill location:** `.claude/skills/bun/`
...
```bash
.claude/scripts/issue-tracker.sh on-deploy "staging" "$(cat <<'EOF'
### Staging Deployment
...
# Hook automatically updates label to "staging-deployed"
```

After production deployment:

```bash
.claude/scripts/issue-tracker.sh on-deploy "production" "$(cat <<'EOF'
...
# Hook automatically updates label to "deployed" and closes issue
```
